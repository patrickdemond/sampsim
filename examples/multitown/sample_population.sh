#!/bin/bash
# 
# This script samples the populations generated by the generate_population.sh script
# 
####################################################
      ...
      for ((i=${#files[@]}-1; i>=0; i--)); do
        ...
        if [ -f "$name.done" ]; then
          ...
          if [ ! -z $sbatch ]; then
            ...
            sbatch $batch_file && touch $name.done
          elif [ ! -z $sqsub ]; then
            sqsub -r 120m --mpp=4g -q serial -o $name.log $command && touch $name.done
          else
